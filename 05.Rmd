...
억제제의 농도에 따라 대사체 생성이 얼마나 억제 되었는지를 그래프화 하여 IC~50~를 구한다. (그림 \@ref(fig:05-04))
...
isoform에 대해서 얻을 수 있으며, IC~50~ 값의 해석은 아래와 같다. (표 \@ref(tab:interpretic50))
...
값을 계산할 수 있다(표 \@ref(tab:variousmodels), static model).
...
약물 노출 값의 변화(fold-change in AUC)를 구할 수 있다(표 \@ref(tab:variousmodels), 수식 \@ref(eq:mbiddi), AUC fold-change calculation for mechanistic static model).
