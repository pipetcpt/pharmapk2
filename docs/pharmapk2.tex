...
\caption{Biopharmaceutics Classification System (BCS) (\citeproc{ref-lubrizol}{Lubrizol Life Science 2019})}\label{fig:01-02}
\end{figure}
...
\caption{Kinetic solubility 측정법 모식도 (\citeproc{ref-everythingadme}{Cyprotex 2022})}\label{fig:01-03}
\end{figure}
...
\caption{\label{tab:table01-05} Kinetic solubility의 침전을 검출하는 방법 (\citeproc{ref-di2015drug}{Di and Kerns 2015})}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Nephelometry
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\caption{Thermodynamic solubility 측정법 모식도 (\citeproc{ref-everythingadme}{Cyprotex 2022})}\label{fig:01-04}
\end{figure}
...
\caption{Kinetic solubility와 Thermodynamic solubility의 비교 (\citeproc{ref-alskar2018improved}{Alskar 2018})}\label{fig:01-05}
\end{figure}
...
\caption{Caco-2 cell-based permeability assay 방법 (\citeproc{ref-everythingadme}{Cyprotex 2022})}\label{fig:02-06}
\end{figure}
...
\caption{Caco-2 투과도와 사람 소장 흡수율 간의 상관 관계 (\citeproc{ref-everythingadme}{Cyprotex 2022})}\label{fig:02-08}
\end{figure}
...
\caption{한여과 (ultrafiltration) 평가법 (\citeproc{ref-everythingadme}{Cyprotex 2022})}\label{fig:03-03}
\end{figure}
...
있다 (\citeproc{ref-studies2020cytochrome}{In Vitro Drug Interaction Studies 2020}; \citeproc{ref-european2012guideline}{European Medicines Agency 2012}).
...
확인(identification)하는 것이 권장된다{[}In Vitro Drug Interaction Studies (\citeproc{ref-studies2020cytochrome}{2020})\}. Reaction phenotyping
...
있다{[}In Vitro Drug Interaction Studies (\citeproc{ref-studies2020cytochrome}{2020})\}.
...
\caption{Perpetrator와 victim drug 병용투여 시의 victim drug의
약물농도. Perpetrator: 다른 약의 PK에 영향을 끼치는 약물(억제제 또는
유도제), (a)억제제(inhibitor)인 경우, (b)유도제(inducer)인 경우, Victim
NCE: New Chemical Entity로 자신의 PK가 영향을 받는 약물(기질)}\label{fig:05-01}
\end{figure}
...
\ref{fig:05-20})(\citeproc{ref-li2008drug}{Li 2008})
...
\caption{Michaelis-Menton kinetics of typical enzyme-catalyzed
reaction of one substrate Substrate saturation experiment results for
enzyme kinetic parameters(V\textsubscript{max}, K\textsubscript{m}), plotting reaction rate vs
substrate concentration. V\textsubscript{max}: Maximum reaction rate at saturating
substrate concentration for a given enzyme concentration, K\textsubscript{m}:
Substrate concentration at half-maximum reaction rate(V\textsubscript{max}), also
known as Micahelis constant.}\label{fig:05-03}
\end{figure}
...
\caption{Lineweaver-Burk plots for enzyme inhibition(\citeproc{ref-linearweaver}{Wikipedia 2023}).
위의 그림은 Michaelis-Menton plot(그림 \ref{fig:05-03})의 역수이며,
속도의 역수(1/V)에 대한 기질 농도의 역수(1/{[}S{]})의 비율을 나타냄}\label{fig:05-20}
\end{figure}
...
수행해야한다. 가역적 억제제인지 평가하기 위해 우선 IC\textsubscript{50}를 구해야 한다.
이는 CYP inhibition assay(IC\textsubscript{50})로 구할 수 있다. 이때의 IC\textsubscript{50}는 50\%
억제를 나타낸 억제제의 농도를 의미하며, 다양한 test compound를 재료로
다양한 억제제 농도에서 생성되는 대사체의 양을 측정하여, 궁극적으로
억제제의 농도에 따라 대사체 생성이 얼마나 억제 되었는지를 그래프화 하여
IC\textsubscript{50}를 구한다. (그림 \ref{fig:05-04})

억제제와 기질을 Human liver microsome(HLM)과 함께 preincubate 시킨 후,
조효소(cofactor)인 NADPH를 넣고 incubation을 거친다. 각 억제제의
농도에서 대사체 생성 양을 LC-MS/MS를 통해 관측한다. IC\textsubscript{50} 값은
extrinsic constant 로 lab-to-lab variability 가 존재하고, 재생산하기
어려운 값일 수 있으며, 기질의 종류, 농도 그리고 incubation condition에
따라 달라질 수 있다. 이때 얻어지는 IC\textsubscript{50} 값은 각각의 CYP isoform에
대해서 얻을 수 있으며, IC\textsubscript{50} 값의 해석은 아래와 같다. (표
\ref{tab:interpretic50})
...
\caption{Metabolite formation depending on the various inhibitor
concentration (\citeproc{ref-invitroadme}{Cyprotex 2023})}\label{fig:05-04}
\end{figure}
...
\begin{longtable}[]{@{}ll@{}}
\caption{\label{tab:interpretic50} Interpretation of IC50 values (\citeproc{ref-invitroadme}{Cyprotex 2023})}\tabularnewline
\toprule\noalign{}
...
\caption{Reaction rate (velocity) vs {[}S{]} or v/{[}S{]}. (a),(b):
Competitive inhibition mechanism, (c),(d): Noncompetitive inbition
mechanism, (e),(f): Uncompetitive inhibition mechanism. (b),(d),(f):
Linearized (Eadie-Hofstee) plot. Y-intercept: V\textsubscript{max}, Slope: K\textsubscript{m} and
{[}I{]}: Inhibitor concentration}\label{fig:reactionrate}
\end{figure}
...
types.(\citeproc{ref-invitroadme}{Cyprotex 2023}) V\textsubscript{max} : Maximal rate, K\textsubscript{m} : Affinity constant,
{[}S{]} : Substrance concentration, {[}I{]}: Inhibitor concentration, K\textsubscript{i} :
Inhibition contant, \(\alpha\) : Interaction parameter which determines
the degree to which the binding of inhibitor changes the affinity of the
enzyme of the substrate}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Inhibition type
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
확인한다(그림 \ref{fig:IC50shift}).

이때 얻어지는 그래프는 억제제 농도에 따른 \%CYP 활성이며, NADPH를
포함하여 preincubation 한 경우의 IC\textsubscript{50} 가 약 1.5배 작아지면(left shift
of IC\textsubscript{50}), 해당 약물은 TDI을 나타내는 것으로 해석될 수
있다.(\citeproc{ref-invitroadme}{Cyprotex 2023}) 다만, 해당 수치적 기준은 명확하게 수립된 바는 없으며
IC\textsubscript{50} shift가 대략 2.5-3배 이상이면 TDI를 나타내는 것으로 보기도 한다.
이와 같은 IC\textsubscript{50} 값의 left shift는 preincubation을 통해 CYP 활성을
억제하는 능력이 커짐을 의미한다.
...
\caption{Example of IC50 shift profile of \%CYP activity vs.
inhibitor concentration plus/minus NADPH during preincubation
period(\citeproc{ref-invitroadme}{Cyprotex 2023})}\label{fig:IC50shift}
\end{figure}
...
그림 내 수식)(\citeproc{ref-li2008drug}{Li 2008})
...
\caption{(a) Example of (a) \%CYP activity remaining vs.~preincubation
time, (b) -slope (of (a) graph) vs.~inhibitor concentration or Observed
rate (Kobs) model, (c) Lineweaver-Burk plot, x-intercept: 1/KI,
y-intercept: 1/Kinact (\citeproc{ref-li2008drug}{Li 2008})}\label{fig:05-21}
\end{figure}
...
inhibition assay를 통해 구해진 억제 파라미터를 의미. (\citeproc{ref-li2008drug}{Li 2008})
...
권고하고 있다. (\citeproc{ref-european2012guideline}{European Medicines Agency 2012}; \citeproc{ref-kenna2020sc1}{Kenna 2020}; \citeproc{ref-zamek2018transporters}{Zamek-Gliszczynski et al. 2018})
...
\bibitem[\citeproctext]{ref-alskar2018improved}
...
\bibitem[\citeproctext]{ref-everythingadme}
...
\bibitem[\citeproctext]{ref-lubrizol}
...
\bibitem[\citeproctext]{ref-linearweaver}
