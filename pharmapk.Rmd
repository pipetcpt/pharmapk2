...
[drug]~acceptor~ = Concentration of compound in the acceptor compartment
...
그리고 아래와 같은 식을 이용하여 세포막 투과계수(P~app~, apparent
...
well의 표면적(the surface area of the monolayer, cm^2^), C~0~는 처리한
약물의 농도(initial concentration in the donor chamber, μmol/cm^3^)를
의미한다.
...
판단할 수 있다(기질인 경우 Efflux ratio \> 2). 그러나 세포 배양 시 3주
...
+================+===============+===============+===============+
| **Cell         | Artificial    | Human         | Madin-Darby   |
| origin**       | membrane      | colorectal    | canine kidney |
...
| transporters** |               | OATP1B3,      | and m         |
|                |               | OATP2B1,      | onocarboxylic |
|                |               | PepT1, OCT1,  | acids         |
|                |               | OCT2 and OCT3 |               |
+----------------+---------------+---------------+---------------+
...
: (#tab:table02-01) 투과도 평가 시험 방법 별 특성 비교

-: not applicable.

## 투과도 평가 결과의 해석 및 활용

앞서 설명한 *in vitro* 시험들을 통해 약물의 세포막 투과계수를 산출할 수
있으며, 이를 토대로 그 약물의 소장에서의 흡수 정도를 예측하게 된다. 또한
*in vitro* 활성 시험에 있어서도 화합물이 세포내 표적에 도달하여
...
1×10^-6^ cm/sec 이상의 약물이면 경구용 약물로 개발가능한 수준으로
판단하는 경우도 있다.

Caco-2 시험에서 산출된 apical to basolateral 방향의 겉보기
투과계수(P~app,~ A to B)는 동일한 시험 배치에서 평가한 기준
...
of human intestinal absorption; f~a~)에 따라 high (≥85%), moderate (50 -
84%), low (\<50%) 및 zero-permeable 약물로 분류되어 있다. 보다 정확한
...
방향의 P~app~ 값이 2배 이상 증가한 것을 확인할 수 있다. 그 결과 저해제를
...
P~app~ 및 efflux ratio가 변하지 않고 일정한 값으로 산출되었다. 그러므로
...
read_csv('data-raw/caco-2.csv', col_types="ccccccc") %>% 
  rename(`Papp (A to B) [×10-6 cm/sec]` = 2,
         `Papp (B to A) [×10-6 cm/sec]` = 3,
         `Efflux ratio` = 4,
         `Papp (A to B) [×10-6 cm/sec] ` = 5,
         `Papp (B to A) [×10-6 cm/sec] ` = 6, 
         `Efflux ratio ` = 7) %>% 
...
#  kable_styling(full_width = F) |> 
  ...
  add_footnote(c("P~app~ (A to B): permeability coefficient in apical to basolateral direction", "P~app~ (B to A): permeability coefficient in basolateral to apical direction."), notation = "none")
```
...
활용될 수 있다. 앞서 설명한 바와 같이 투과도를 평가하기 위한 *in vitro*
...
3.  Di, Li, and Edward Kerns. *Drug-like properties: concepts, structure
    ...
    press (2015).
4.  Cyprotex Ltd. Everything you need to know about ADME, 3^rd^ Edition
    (2017).
...
7.  Saxena, Amrita, et al. "Pharmacokinetics, dose proportionality and
    ...
    1016-1023.
8.  Masungi, C., et al. "Parallel artificial membrane permeability assay
    (PAMPA) combined with a 10-day multiscreen Caco-2 cell culture as a
    tool for assessing new drug candidates." *Die Pharmazie-An
...
C~max~와 AUC는 cimetidine과 병용 시 각각 1.5배 및 1.6배 증가하였다. [@wang2008oct2] (그림 \@ref(fig:07-09))
